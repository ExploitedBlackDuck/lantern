...
final class GitBinary {
	...
	/**
	 * Per-invocation `-c` overrides that disable every repo-config directive
	 * which can run an external program, so even an untrusted repo (§9.6a)
	 * cannot escalate a read into code execution:
	 *  - core.fsmonitor=        : no file-system-monitor daemon is spawned
	 *  - core.hooksPath=/dev/null: no hook is ever found/run
	 *  - core.attributesFile=/dev/null + core.attributesfile=… : ignore a global
	 *    gitattributes (textconv/filter drivers); repo .gitattributes still
	 *    can't bite because we never pass --textconv / run diff filters
	 * diff.external= is included so the H4 diff/show feature cannot be turned
	 * into code execution by a repo-controlled external diff driver (§9.6).
	 */
	private const HARDENING_FLAGS = [
		'-c', 'core.fsmonitor=',
	...
	public function run(string $repoPath, array $args, ?int $maxBytes = null): GitResult {
		...
		//
		// HARDENING_FLAGS additionally neutralise the repo-local config
		// directives that can execute an external program — git ALWAYS honors a
		// repo's own .git/config (§9.6), so for an UNTRUSTED repo (the v2
		// user-Files case, §9.6a) command selection alone is not enough. These
		// per-call overrides disable the fsmonitor daemon, hooks, and the global
		// attributes file so no .git/config or .gitattributes can turn a
		// read-only browse into code execution. Harmless for trusted repos too.
		$command = array_merge(
			[$this->gitPath, '-C', $repoPath, '-c', 'safe.directory=' . $repoPath],
